...
sys.path.append('C:\\Users\\kholb\\source\\repos\\Working\\frontend-sdl-py\\dll')
...
if os.name == "nt":
    PresetSwitchedCallback = ctypes.WINFUNCTYPE(None, ctypes.c_bool, ctypes.c_uint, ctypes.c_void_p)
    PresetSwitchFailedCallback = ctypes.WINFUNCTYPE(None, ctypes.c_char_p, ctypes.c_void_p)
else:
...
class ProjectMWrapper:
    def __init__(self, config, sdl_rendering):
        ...
        try:
            ...
            log.warning('Failed to load projectM-4 library using default paths, attempting to use config-specified path...')
            projectm_dll_path = self.config.projectm.get(
                    "projectm.projectmlib", 
                    "/usr/local/lib/libprojectM-4.so"
                )

            if os.name == "nt":
                self.projectm_lib = ctypes.WinDLL(projectm_dll_path)
            else:
                self.projectm_lib = ctypes.CDLL(projectm_dll_path)
        
        self.projectm_playlist = None
        try:
            self.projectm_playlist_lib = load_library('projectM-4-playlist')
        except:
            log.warning('Failed to load projectM-4-playlist library using default paths, attempting to use config-specified path...')
            projectm_playlist_dll_path = self.config.projectm.get(
                    "projectm.projectmplaylistlib", 
                    "/usr/local/lib/libprojectM-4-playlist.so"
                )

            if os.name == "nt":
                self.projectm_playlist_lib = ctypes.WinDLL(projectm_playlist_dll_path)
            else:
                self.projectm_playlist_lib = ctypes.CDLL(projectm_playlist_dll_path)
                    ...
                    for root, dirs, files in os.walk(preset_path):
                        for name in files:
                            ...
                            if not preset_path in presets:
                                presets.append(str(Path(preset_path)))

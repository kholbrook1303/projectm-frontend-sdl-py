...
class AudioCaptureImpl:
    ...
    def start_recording(self, index):
        ...
        try:
            log.info("Current audio driver: {}".format(sdl2.SDL_GetCurrentAudioDriver().decode()))
    ...
    def open_audio_device(self):
        ...
        if self.currentAudioDeviceID == 0:
            log.error(f'Failed to open audio device "{deviceName}" (ID {self.currentAudioDeviceIndex}): {sdl2.SDL_GetError()}')
            return False

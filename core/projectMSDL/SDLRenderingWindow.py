...
class SDLRenderingWindow:
    def __init__(self, config):
        ...
        self.verify_opengl_features()

        self.create_sdl_window()

    def verify_opengl_features(self):
        sdl2.SDL_Init(sdl2.SDL_INIT_VIDEO)
    
        window = sdl2.SDL_CreateWindow(
            b"Open GL Version/Extension Check", 
    ...
    def fullscreen(self):
        ...
        if self.config.projectm.get('window.fullscreen.exclusivemode', False):
    ...
    def create_sdl_window(self):
        ...
        # if self.config.projectm.get('window.hardwareacceleration', True):
        #     rendering_flags = sdl2.render.SDL_RENDERER_ACCELERATED | sdl2.render.SDL_RENDERER_PRESENTVSYNC
        # else:
        #     rendering_flags = sdl2.render.SDL_RENDERER_SOFTWARE
        ...
        if self.use_glew:
            glew_dll = self.config.projectm.get('projectm.projectmglewlib', 'dll/glew32.dll')
            glewInit = ctypes.CDLL(glew_dll).glewInit
            glewInit.restype = ctypes.c_uint
            if glewInit() != 0:
                print("glewInit failed")
                return -1
